...
- **Timers** — active buffs at their exact server durations and recasts on one panel, color-ramped by urgency. Buffs you cast on the party fold into AoE-counted rows, with bard song tags, corsair roll pips and geomancer aura lifetimes. A family-organised buff filter — one 4-state setting per buff (Show / Show+alert / Hide / Hide+alert), shared across every job — blinks a warning as a critical buff (Haste, Refresh, Phalanx, Composure, Reraise…) runs low and flags it when it drops, and a buff-source filter keeps only your own, players' or trusts' buffs. Recasts are always shown.
