# gen_th_icon.ps1 -- build the Treasure Hunter coffer icon from a full-colour illustration.
#
# Source : an RGBA treasure-chest illustration whose background is ALREADY transparent (alpha 0).
#          Default: assets/icon_th_src/tresor.png (the source of record); override with -Src.
# Output : assets\icon_th_coffer.raw -- 64x64 straight-alpha BGRA blob for load_raw_texture().
#
...
#      transparent pixels merely carry leftover olive RGB, which must be ignored).
#   2. Trim to the chest bounding box (alpha > 8).
#   3. Fit CENTERED into 64x64 with a ~2px transparent margin, aspect preserved (letterboxed --
#      the chest is wider than tall), downscaled with LANCZOS in PREMULTIPLIED-alpha space so the
...
#
# Pure asset tooling -- no game/RE involved. Re-run if the source coffer art changes.

param(
    [string]$Src = (Join-Path (Split-Path $PSScriptRoot -Parent) 'assets\icon_th_src\tresor.png'),
    [string]$Out = (Join-Path (Split-Path $PSScriptRoot -Parent) 'assets\icon_th_coffer.raw')
)
...
# --- 2. trim to the chest bounding box (by the source's own alpha) --------------------------
...
print("[th-icon] chest bbox %dx%d @(%d,%d)  fit %dx%d @(%d,%d)  bytes=%d"
      % (cw, ch, x0, y0, tw, th, ox, oy, len(bgra)))
print("[th-icon] corner alpha TL,TR,BL,BR = %d,%d,%d,%d   center = %d"
      % (A(0, 0), A(0, SIZE - 1), A(SIZE - 1, 0), A(SIZE - 1, SIZE - 1), A(SIZE // 2, SIZE // 2)))
'@

$tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("gen_th_icon_{0}.py" -f ([guid]::NewGuid().ToString('N')))
try {
...
Write-Host "[th-icon] OK -> $Out  ($len bytes)  64x64 straight-alpha BGRA"
