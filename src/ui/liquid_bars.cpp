...
static const char* CAP_FRONT() { static char b[260]; if (!b[0]) plugin_path(b, 260, "assets\\cap_front.bin"); return b; }
static const char* CAP_BACK() { static char b[260]; if (!b[0]) plugin_path(b, 260, "assets\\cap_back.bin"); return b; }
...
// public wrapper : the same glass front-face for external gauges (Target distance). Leaves DESTBLEND additive.
void fiole_glass(u32 dev, float x, float y, float w, float h) { glass_overlay(dev, x, y, w, h); }
...
#pragma warning(push)
#pragma warning(disable : 4702)             // draw() is intentionally HIDDEN (early return) ; the body is kept for re-enabling -> its "unreachable" lines are expected, not a bug
...
// ---- rounded stencil CLIP (rrect_clip_begin/end) -> gfx/draw.h (shared with target.cpp) ----
