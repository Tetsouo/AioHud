...
void pointwatch_draw(const Frame& f, bool preview, float ovX, float ovY, float ovS, float screenW, float screenH,
    ...
    else            { px = C.pwX * screenW - boxW * 0.5f; py = C.pwY * screenH; }
    if (editing) { static EditBox g_pwEdit; box_edit(f, g_pwEdit, EDITBOX_POINTWATCH, px, py, boxW, boxH, ui_config().pwScale, ui_config().pwX, ui_config().pwY, true); }
