...
void pointwatch_draw(const Frame& f, bool preview, float ovX, float ovY, float ovS, float screenW, float screenH,
    ...
    float px, py;
    if (ovS > 0.0f) { px = ovX - boxW * 0.5f; py = ovY - boxH * 0.5f; }        // preview/override : centred in the stage
    else            { px = C.pwX * screenW - boxW; py = C.pwY * screenH; }      // live : RIGHT-anchored (pwX = right edge) -> width changes move the LEFT edge only
