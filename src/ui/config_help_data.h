...
static const HelpItem HELP_TIMERS[] = {
    ...
    {2, "*Hidden + focus* : hidden while healthy, but *pops a red alert* once it drops under the *Focus: warn under* threshold, or when it falls / is dispelled. The alert holds for the *Focus: hold alert* duration then clears itself if you don't re-cast. Both are sliders just above. Zero clutter, zero misses.",
        "*Masqué + focus* : caché tant que tout va bien, mais *surgit en alerte rouge* dès qu'il passe sous le seuil *Focus : alerter sous*, ou qu'il tombe / est dispel. L'alerte tient le temps réglé par *Focus : durée alerte* puis s'efface si tu ne relances pas. Les deux sont des curseurs juste au-dessus. Zéro encombrement, zéro oubli."},
    ...
    {2, "Song modifier tags (*SV*, *NT*, *TR*, *M*) only ever appear on a song *you* cast — they describe your own job abilities at cast time.",
        "Les marqueurs de chants (*SV*, *NT*, *TR*, *M*) n'apparaissent que sur un chant que *tu* as lancé — ils décrivent tes propres aptitudes au moment de l'incantation."},
