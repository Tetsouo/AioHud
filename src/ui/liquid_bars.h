...
// the fiole's GLASS front-face (curvature shading + specular), for gauges that want the HP-bar glass look
// (e.g. the Target distance gauge). LEAVES DESTBLEND additive -> the caller must reset (setup_color_state / cs()).
void        fiole_glass(u32 dev, float x, float y, float w, float h);
