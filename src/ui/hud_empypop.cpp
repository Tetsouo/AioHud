...
void empypop_draw(const Frame& f, bool preview, float ovX, float ovY, float ovS, float screenW, float screenH,
    ...
    float px, py;
    if (ovS > 0.0f) { px = ovX - boxW * 0.5f; py = ovY - boxH * 0.5f; }   // preview / Help : centred on the stage
    else            { px = C.epX * screenW - boxW; py = C.epY * screenH; }   // epX = the RIGHT edge : the box grows LEFT (and down) as content widens, so the top-RIGHT corner stays where it was dropped
